    ...
    async redirectToAuthorization(authorizationUrl) {
      ...
      return withAuthLock(async () => {
        ...
        if (!allowBrowser) {
          ...
          // Keep the loopback listener alive so the returned authorization
          // link still completes in the background: once the user opens the
          // URL and authorizes, the code below exchanges and stores tokens,
          // and the next connect (or a manual refresh) succeeds. The listener
          // expires after 5 minutes so it never leaks on the stable port.
          log.info(`${tag}: mount requires authorization, not opening browser`);
          const authUrl = authorizationUrl.toString();
          const timer = setTimeout(() => {
            log.info(`${tag}: no-browser authorization listener expired`);
            server.close();
          }, 5 * 60e3);
          timer.unref?.();
          void resultPromise.then(async (result) => {
            try {
              ...
              // saveClientInformation, so the closure copy is null and the
              // token request would lack client_id (CAS: "... are required").
              ...
              log.info(`${tag}: tokens saved (no-browser mount flow)`);
            } catch (error) {
              log.error(`${tag}: background mount authorization failed: ${error instanceof Error ? error.message : String(error)}`);
            } finally {
              clearTimeout(timer);
              server.close();
