...
- **OAuth 授权链接未在挂载失败信息中展示**：mcp-client 的启动包装错误把底层连接错误（含授权链接）塞进 `cause`，而挂载失败视图只显示 message，导致 v1.9.0 的授权链接提醒不生效。修复：底层错误详情并入 message

## [1.9.0] - 2026-08-26

### 新增

- **OAuth 挂载失败附授权链接**：挂载需要授权时不再只给指引文案，失败信息直接附带**可点击的授权链接**（后台回调监听器保持运行，打开链接完成授权后自动保存 token，下次挂载/刷新即成功）
...
- **stdio 表单参数输入改进**：参数支持 shell 风格解析（空格/换行分隔，支持引号、转义与显式空参数），可直接粘贴命令行（如 `-y @modelcontextprotocol/server-filesystem /path/to/dir`）；参数框下方实时预览解析结果，保存前即可发现参数拆分问题
