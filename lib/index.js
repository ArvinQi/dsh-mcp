...
let McpManagerService = (() => {
	...
	return class McpManagerService extends _classSuper {
		static {
			...
			_toolsSetJson_decorators = [Remote("toolsSetJson")];
			...
			__esDecorate(this, null, _toolsSetJson_decorators, {
				kind: "method",
				name: "toolsSetJson",
				static: false,
				private: false,
				access: {
					has: (obj) => "toolsSetJson" in obj,
					get: (obj) => obj.toolsSetJson
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
		...
		*/
		...
		/**
		* Apply a batch of tool enable switches in one call (JSON editor path).
		* The client parses its nested `{server: {tool: enabled}}` text into a
		* flat `{fullName: boolean}` mapping and submits it here.
		* @param request - Flat tool-name -> enabled mapping.
		* @returns the applied switch count, or a failure.
		*/
		async toolsSetJson(request) {
			const switches = request?.switches;
			if (switches === null || typeof switches !== "object" || Array.isArray(switches)) {
				return {
					ok: false,
					error: {
						code: "MCP_TOOL_INVALID_JSON",
						message: "switches must be an object mapping mcp__ tool names to booleans"
					}
				};
			}
			const entries = Object.entries(switches);
			for (const [name, enabled] of entries) {
				if (typeof name !== "string" || !name.startsWith("mcp__") || typeof enabled !== "boolean") {
					return {
						ok: false,
						error: {
							code: "MCP_TOOL_INVALID_JSON",
							message: `invalid switch entry ${JSON.stringify(name)}: expected a mcp__ tool name with a boolean value`
						}
					};
				}
			}
			for (const [name, enabled] of entries) this.toolSwitches.set(name, enabled);
