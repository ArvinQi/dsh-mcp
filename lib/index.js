...
let McpManagerService = (() => {
	...
	return class McpManagerService extends _classSuper {
		...
		*/
		...
		/**
		* Resolve the process-level env map. The env table only registers
		* variable names; values come from the process environment (process.env)
		* by the same name — the editor never stores values.
		*/
		async resolveGlobalEnv() {
			const env = {};
			for (const [name] of this.requireEnvTable().entries()) if (process.env[name] !== void 0) env[name] = process.env[name];
			return env;
		}
		...
		*/
		...
		/**
		* Read the registered process-level environment variable names. Values
		* are not stored: they come from the process environment (process.env)
		* by the same name at substitution time.
		...
		*/
		...
		/**
		* Replace the whole process-level env registration (names only). Values
		* always come from process.env; no value or secret is persisted.
		* @param request - The full desired variable names.
		...
		async envSet(request) {
			...
			if (!Array.isArray(inputs)) {
				return {
					...
					error: {
						code: "MCP_INVALID_SPEC",
						message: "vars must be an array of { name }"
					}
				};
			}
			const table = this.requireEnvTable();
			...
			// Rewrite the table with names only; drop any legacy stored values.
			for (const name of [...table.entries()].map(([key]) => key)) {
				if (!names.has(name)) await this.ctx.credentials.unset(globalEnvRef(name));
				await table.delete(name);
			}
			for (const entry of inputs) {
				await table.put(entry.name, { name: entry.name, secret: false, value: "" });
			}
