...
function startConnection(ctx, config, policy) {
	...
	const opts = {
		...
		// Read by the per-tool-call OAuth guard: the server URL drives discovery,
		// and the provider reports whether a stored token is still usable.
		url: config.url,
	...
	*/
	async function connectGeneration(startup) {
		// SDK 2.0 no longer accepts `authProvider` in ClientOptions; the transport
		// created by createTransport(config) carries it (config.authProvider).
		...
		generation.setNotificationHandler("notifications/tools/list_changed", async () => {
			if (!isCurrent(generation)) return;
