...
/**
* Substitute `${NAME}` placeholders in one header value with the server's
* resolved env (including secrets) or the process environment. Unresolved
* placeholders stay literal so a missing variable never silently empties a
* header. Also supports `$${NAME}`? No — a plain `${NAME}` is enough; an
* unknown variable keeps the placeholder for visibility.
* @param value - The stored header value (may contain `${NAME}` tokens).
		...
		*/
		...
		toClientConfig(spec, env) {
			...
			return {
				...
				serverName: spec.serverName,
				url: spec.url,
				// Header values support `${ENV}` placeholders resolved from the
				// server's env (including secrets) or the process environment.
				headers: Object.fromEntries(spec.headers.map((header) => [header.name, interpolateEnv(header.value, env)])),
				toolCallTimeoutMs: spec.toolCallTimeoutMs,
				failOnStartupError: spec.failOnStartupError
			};
		}
