    ...
    async redirectToAuthorization(authorizationUrl) {
      ...
      return withAuthLock(async () => {
        ...
        if (!allowBrowser) {
          ...
          // close the listener and fail with guidance (the user authorizes
          // via Test connection, which uses allowBrowser=true).
          log.info(`${tag}: mount requires authorization, not opening browser`);
          server.close();
          throw new Error(`服务器 "${serverId}" 需要 OAuth 授权：请在 Settings → MCP 编辑该服务器并点击「测试连接」完成授权`);
        }
