...
let McpManagerService = (() => {
	...
	return class McpManagerService extends _classSuper {
		...
		*/
		...
		/** Resolve the process-level env map: plain table values plus credentials. */
		async resolveGlobalEnv() {
			const env = {};
			for (const [name, row] of this.requireEnvTable().entries()) if (row.secret) {
				const hit = await this.ctx.credentials.resolve(globalEnvRef(name));
				if (hit !== void 0) env[name] = hit.value;
			} else if (row.value !== void 0) env[name] = row.value;
			return env;
		}
		...
		*/
		...
		/**
		* Read the process-level environment variables (plain values from the env
		* table, secret flags and configured state from the credentials document).
		* Used as the header-substitution source for every server.
		...
		*/
		async envList() {
			...
			for (const [name, row] of table.entries()) {
				const configured = row.secret ? (await this.ctx.credentials.describe(globalEnvRef(name))).configured : (row.value ?? "").length > 0;
				vars.push({
					name,
					secret: row.secret,
					configured,
